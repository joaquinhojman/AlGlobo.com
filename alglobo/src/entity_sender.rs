...
impl EntitySender {
    pub fn new(
        stream: UdpSocket,
        address_map: HashMap<EntityType, String>,
        transactionality_helper_addr: Addr<TransactionCoordinator>,
        statistics_handler: Addr<StatisticsHandler>,
    ) -> Self {
        EntitySender {
            stream,
            address_map,
            coordinator_addr: transactionality_helper_addr,
            statistics_handler,
...
impl Handler<PrepareTransaction> for EntitySender {
    ...
    fn handle(&mut self, msg: PrepareTransaction, ctx: &mut Self::Context) -> Self::Result {
        ...
        self.transaction_timestamps.insert(msg.transaction.get_transaction_id(), Instant::now());
        self.statistics_handler.do_send(
            RegisterTransaction::new(msg.transaction.get_transaction_id())
        );
    }
}
...
impl Handler<BroadcastTransactionState> for EntitySender {
    ...
    fn handle(&mut self, msg: BroadcastTransactionState, ctx: &mut Self::Context) -> Self::Result {
        ...
        if msg.should_await_next_response {
            ...
            let instant = self.transaction_timestamps.get_mut(&msg.transaction_id).unwrap();
            let duration = instant.elapsed();
            self.statistics_handler.do_send(UnregisterTransaction::new(msg.transaction_id, duration))
        }
